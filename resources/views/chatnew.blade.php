...
<div class="modal fade" id="viewDoc" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <!-- Modal: modalCart -->
  <div id="guseradd" class="modal fade" id="modalCart" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <!--Header-->
        ...
        <form action="/group/adduser" method="post">
          @csrf
          <!--Body-->
          <input id="gid" type="hidden" name="gid" value='@if(isset($user)){{$user->id}}@endif'>
          <div class="modal-body overflow-auto" style="height: 70vh">
            <div class="row">
              <div class="border w-100 my-3 p-2">Users</div>
              @foreach($users as $da)
              <div class="d-flex justify-content-between">
                <img class="avatar rounded-circle my-2" src="/storage/images/{{ $da->image }}">
                <h5 class="text-capitalize mt-3">{{$da->fname}}</h5>
                <input type="checkbox" class="mt-3 big-checkbox" name="{{$da->id}}">
              </div>
              @endforeach
            </div>
          </div>
          <!--Footer-->
          <div class="modal-footer">
            <button class="btn btn-info text-white"><i class="fa-regular fa-paper-plane"></i> submit</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div class="navigation navbar justify-content-center py-xl-4 py-md-3 py-0 px-3">
      ...
      <div class="nav flex-md-column nav-pills flex-grow-1" role="tablist" aria-orientation="vertical">
        <a class="mt-xl-3 mt-md-2 nav-link z active" data-toggle="pill" href="#nav-tab-chat" role="tab" title="Chats">
          <i class="ti-comments d-flex flex-column-reverse"><span class='p-1' style='font-size: 14px !important'>{{$munseen ? $munseen : ''}}</span></i>
        </a>
        ...
        <div class="tab-pane fade show active" id="nav-tab-chat" role="tabpanel">
          ...
          <ul class="chat-list" id="myUL">
            ...
            <li id={{$da->id}} @if($da->status == 1) class="online card li-status" @else class="away card" @endif>
              ...
              <a onclick="select('{{$da->name}}','{{encrypt($da->id)}}')" class="card border-0">
            ...
            <li id={{$g->id}} class="li-status">
              ...
              <a onclick="gselect('{{$g->name}}','{{encrypt($g->id)}}')" class="card border-0">
                <div class="card-body">
                  <div class="media">
                    <div class="avatar me-3">
                  ...
                  <div class="media">
                    ...
                    <a href="/select/{{encrypt($da->id)}}" class="w-100">
                      <div class="media-body overflow-hidden">
                        ...
                        <div class="text-truncate text-white d-flex justify-content-between float-end">
        ...
        <div class="overflow-auto">
          ...
          <div class="chat-footer border-top py-xl-0 py-lg-0 pb-2 mb-4">
            <div id='emojiselect' class='row bg-dark d-none'>
                     ...
                     <div onclick='changeEmoji(this, "{{str_replace(array('&', ' '),array('', ''),$group->groups)}}")' class='col @if($group->groups == "Smileys & Emotion") bg-secondary @endif groups fs-1' style='cursor:pointer'>{{json_decode($group->emoji)}}</div>
                     @endforeach
            </div>
            <div id="previewemoji" class='d-none'>
                <div class="fs-1 position-relative">
                  ...
                  @foreach ($emojis as $emoji)
                        ...
                        value="{{json_decode($emoji->emoji)}}">{{json_decode($emoji->emoji)}}</button>
                  @endforeach
                </div>
            </div>
            <div class="container-xxl">
        ...
        <div class="d-flex flex-column">
          ...
          <div class="body mt-4">
            ...
            <h6 class='d-flex justify-content-between mb-4'>
                ...
                <i class="fa-solid fa-user-plus cursor-pointer fs-4 text-secondary" data-bs-toggle="modal" data-bs-target="#guseradd"></i>
                @endif
            </h6>
                ...
                <div class="text-start my-3 border-bottom pb-2">
                  <div class="row">
                    ...
                    <div class="col-lg-3 d-flex justify-content-around">
                        @if($profile->role && $profile->role == "admin")
                          <form action='/group/user-delete' method='post'>
                              @csrf
                              <input type='hidden' name='gid' value='{{$peoples->group_id}}'>
                              <input type='hidden' name='uid' value='{{$peoples->user_id}}'>
                              <button class='border border-0 bg-transparent' onclick='return confirm("Are you sure ?")'><i title='Remove' class="fa-solid fa-user-slash text-danger"></i></button>
                          </form>
                          <form action='/group/make-admin' method='post'>
                              @csrf
                              <input type='hidden' name='gid' value='{{$peoples->group_id}}'>
                              <input type='hidden' name='uid' value='{{$peoples->user_id}}'>
                              <button onclick='return confirm("Are you sure ?")'class='border border-0 bg-transparent'><i title='@if($peoples->grole == 'admin') Make Member @else Make Admin @endif' class="@if($peoples->grole == 'admin') fa-user-gear @else fa-user @endif text-secondary fa-solid"></i></button>
                          </form>
                        @endif
                    </div>
                  </div>
